...
int rok;            //Рокировка
int rok2 = 0;       // для возврата короля на своё место при неудачной рокировке
...
bool check=false;         // флаг определения ШАХ
bool chek2;                // есть ли шах королю соперника, после хода фигуры не объявляющей шах
bool matt=false;         // флаг определения ШАХ
bool ver;
bool pat = true;
bool roki;              // флаг рокировки
int A1, A2;                // если есть шах по от фигуры объявдяющей будем передавать координаты в March и далееж
...
int counterK = 0;
int k1, k2;            // координаты короля для выявления пат
...
//    Проверяет на возможность хода, делает ход, проверяет на МАТ и ПАТ и ШАХ                  НУЖНО ДОБАВИТЬ ФЛАГ НА ЛОКИРОВКУ, ЧТОБЫ ПОТОМ НЕ ПРОВЕРЯЛОСЬ УСЛОВИЯ ОСТАЛЬНЫЕ И ПОДУМАТЬ КУДА ПОСТАВИТЬ ФЛАГ НА ШАХ ЧТОБЫ ЕСЛИ ОН ТРУ ЛОКИРОВКУ НЕЛЬЗЯ БЫЛО ДЕЛАТЬ
